...
def test_api_parse_douyin_url_falls_back_to_ytdlp(monkeypatch):
    """2026-09-17 决策调整：lieshouyin.com 在用户网络下 DNS 层不可达，
    回退到 yt-dlp 自带的 douyin extractor。抖音 URL 也走 yt-dlp.parse_video。
    这里 monkeypatch 替换 ytdlp_service.parse_video，避免真实网络请求。"""
    ...
    assert r.json()["extractor"] == "Douyin"
